...
describe('Claimant court proposed date Controller', () => {
  ...
  describe('on GET', () => {
    ...
    it('should preselect the decision already saved on the claimant response', async () => {
  ...
  describe('on POST', () => {
    ...
    it('should return error on empty post', async () => {
      await request(app).post(CLAIMANT_RESPONSE_COURT_OFFERED_SET_DATE_URL)
        ...
        .expect((res) => {
          expect(res.status).toBe(200);
          expect(res.text).toContain(t('ERRORS.VALID_YES_NO_SELECTION_VARIATION1'));
        });
    });
    ...
    it('should redirect to rejection reason if JUDGE_REPAYMENT_DATE is selected', async () => {
      await request(app).post(CLAIMANT_RESPONSE_COURT_OFFERED_SET_DATE_URL)
    ...
    it('should redirect to task list if ACCEPT_REPAYMENT_DATE is selected', async () => {
      await request(app).post(CLAIMANT_RESPONSE_COURT_OFFERED_SET_DATE_URL)
