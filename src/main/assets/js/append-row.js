import {getCalculation, addCalculationEventListener} from './calculate-amount.js';
import {addTotalClaimAmountCalculationEventListener} from './calculate-total-amount.js';
...
/**
 * Repeatable rows for timeline, expenses, employers, and directions questionnaire.
 *
 * Markup: a `.append-row` button and `.row-container` rows. This is a different
 * DOM contract from {@link initAddAnother} (`cui-add-another*` / `%index%`
 * placeholders). `initAddAnother` starts this function so the webpack entry
 * has one init path.
 *
 * @param scope - Document or container to search
 */
export function initAppendRow(scope = document) {
  const appendRowButton = scope.querySelector('.append-row');
  if (!appendRowButton) {
    return;
  }
