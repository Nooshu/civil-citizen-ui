...
`yarn-audit-known-issues` records **accepted toolchain** advisories (CodeceptJS, Mocha, Sass, Pact, Jest’s nested CommonJS glob 7 / `inflight`, glob 8 via `help-me`, and similar). Do not delete it to “go green” without review, and do not add a production-tree finding there — those must be upgraded instead.
