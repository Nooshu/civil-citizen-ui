...
| Find address | Ordnance Survey postcode client | GET | `/search/places/v1/postcode` | `postcode=MK5 7HH` | API key is supplied but not behaviour-selecting | None | 200 | `results[].DPA` Delivery Point Address fields | `create-claim-address-search.json` | `create-claim-address.json` | CUI | External API; no Civil Service Pact |
