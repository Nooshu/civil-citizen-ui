...
Pa11y. Real command: `yarn tests:a11y`. HTML fixtures under `src/test/utils/mocks/a11y/` (claim `1645882162449409`). `ignored-urls.ts` / `action-urls.ts` — do not ignore new pages without reason. `yarn test:a11y` is a **stub**.
