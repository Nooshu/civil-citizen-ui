...
| `renovate.json` | `rangeStrategy: pin`; npm `minimumReleaseAge` 7 days; HMCTS `automerge-minor` (not `automerge-all`). The Jenkins Library preset was removed (global config). Majors and `govuk-frontend` do not automerge. Security alerts skip the age gate. Do not re-add ranged bumps or major automerge. |
