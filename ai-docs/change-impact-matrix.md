...
Exact pins for packages you touch. 7-day cooldown unless security. Then **one** `yarn test:coverage`. SIGSEGV → re-run that file only. Package-**only** changes may follow the origin auto-push rule in `AGENTS.md`.
