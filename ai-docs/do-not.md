...
- Do not hand-write GOV.UK component HTML (`govuk-button`, `govuk-error-summary`, `govuk-header`, `govuk-table`, `govuk-inset-text`, `govuk-tabs`, `govuk-summary-list`, `govuk-tag`, footer, skip link, breadcrumbs) when a Nunjucks macro exists.
...
- Do not treat a catalogue HTTP 200 as a finished screen. Do not fill empty preview pages with hard-coded sample copy in Nunjucks — seed WireMock/Redis or add a production-safe fallback ([playbooks/ui-preview-missing-data.md](playbooks/ui-preview-missing-data.md)). Grow the catalogue by fixing GETs that still 500 or look empty (`£NaN`, `Invalid DateTime`, `Created []`), not by adding links toward a count.
