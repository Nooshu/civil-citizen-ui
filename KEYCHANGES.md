...
The catalogue lists **200+** live Nunjucks screens (eligibility, claim issue, response, statement of means, mediation, directions questionnaire, claimant response, case progression, general application, query management). Full- and part-admit seeds include an instalment repayment plan (£100 a month from 18 September 2026), so **How they want to pay** shows amount, frequency, first and final dates, and length. Claim `1645882162449605` is a defendant part-admit so statement-of-means pages pass their guard. Case progression `1645882162449603` is `FAST_CLAIM` with trial-arrangement answers, a 60-minute hearing, and a Help with Fees reference. Claim `1645882162449604` seeds a strike-out general application draft. WireMock also stubs court locations, Ordnance Survey postcode lookup, airlines, fee ranges, general-application fees, the GA case, and a sample dashboard task list. Links that cannot GET-render (unregistered URLs, PIN sessions, incomplete CYA) are omitted from the catalogue.
...
**3. What you look at is GOV.UK, not a lookalike.** Frontend is on 6.4. Nineteen templates that used hand-written header, table, inset, or button markup now call official macros. A shared fragment carries that into fifty-nine other screens. Later passes put tabs, summary lists and cards, tags, Find address, the paper-form address, radios fieldsets, date errors, details, and task lists on the same macros. One pin bump plus the fixture suite absorbs Design System changes instead of rewriting pages. That is cheaper maintenance, aligned branding, and accessibility that comes from GOV.UK — focus, labels, keyboard including tabs, table structure. We prefer GOV.UK over axe when they conflict.
...
| Touch tabs, tags, summary lists, Find address, radios, dates, or task lists | Official `govukTabs`, `govukTag`, `govukSummaryList`, `govukRadios`, `govukDateInput`, `govukTaskList`, `govukDetails`, `govukInput`, and `govukButton` |
